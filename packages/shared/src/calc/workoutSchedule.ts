...
function cardioLabel(location: WorkoutLocation, sport: string | null): string {
  ...
  if (location === "outside") return "Outdoor cardio";
  if (location === "gym") return "Cardio machines";
  return "Home cardio";
}
...
export function buildWorkoutSchedule(input: WorkoutScheduleInput, sport: string | null = null): WorkoutDay[] {
  ...
  for (const day of order) {
    if (strengthSlots.has(day)) {
      const label =
        strengthCount === 4
          ? PUSH_PULL_LEGS[strengthIndex % PUSH_PULL_LEGS.length]
          : "Full-body strength";
      byDay.set(day, {
        day,
        kind: "strength",
        label: `${label} — ${input.equipment} (${input.location})`,
      });
      strengthIndex++;
      ...
      byDay.set(day, { day, kind: "rest", label: "Rest day" });
    }
  }
